...
async function getDataCartDetail() {
    ...
    const response = await fetch('http://100.126.61.16:8080/cart-detail', {
        method: 'GET',
        headers: {
    ...
    tableRow.forEach(function(element){
        ...
        buttonLeft.addEventListener('click', async function(){
            if(numberProduct.textContent != "1"){
                ...
                checkAccessTokenIsvalid();
                let accessToken = localStorage.getItem('accessToken');
                await fetch(`http://100.126.61.16:8080/cart-detail/update`,{
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${accessToken}`
...
function deleteProduct(){
    ...
    for(let i = 0; i < tableRow.length; i++){
        ...
        deleteButton.addEventListener('click', async function(){
            ...
            checkAccessTokenIsvalid();
            let accessToken = localStorage.getItem('accessToken');
            await fetch(`http://100.126.61.16:8080/cart-detail/delete/${id}`,{
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
...
function deleteAllProduct(){
    ...
    buttonClear.addEventListener('click', async function(){
        ...
        checkAccessTokenIsvalid();
        let accessToken = localStorage.getItem('accessToken');
        await fetch(`http://100.126.61.16:8080/cart-detail/deleteUserCart`, {
            method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
...
function creatOrder(){
    ...
    buttonCreatOrder.addEventListener('click', async function(){
        ...
        if(ok1 == 1 && ok2 == 1){
            ...
            checkAccessTokenIsvalid();
            let accessToken = localStorage.getItem('accessToken');
            await fetch(`http://100.126.61.16:8080/order/createOrderFromCart`, {
                method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${accessToken}`
            ...
            await fetch(`http://100.126.61.16:8080/order-detail/add`, {
                method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${accessToken}`
