...
async function changeUserInfor() {
    thongTinTaiKhoan.addEventListener('click', async function(){
        ...
        button.addEventListener('click', async function(){
            ...
            await checkAccessTokenIsvalid();
            await fetch('http://100.126.61.16:8080/user/changeInfo',{
                method: 'PUT',
                headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${accessToken}`
...
async function buildOder(){
    ...
    danhSachDonHang.addEventListener('click', async function(){
        ...
        await checkAccessTokenIsvalid();
        let accessToken = localStorage.getItem('accessToken');
        let response = await fetch('http://100.126.61.16:8080/order/getCurrentUserOrder', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${accessToken}`
...
async function getDataOrderDetail(id, idOrder, receiverName, receiverPhone, shippingAddress, note, status, totalPrice, orderDate){
    ...
    await checkAccessTokenIsvalid();
    let accessToken = localStorage.getItem('accessToken');
    let response = await fetch(`http://100.126.61.16:8080/order-detail/${idOrder}`, {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`
        } 
...
async function changePassword(){
    ...
    thayDoiMatKhauButton.addEventListener('click', async function(){
        ...
        if(newPass1 != newPass2){
            ...
            let response = await fetch('http://100.126.61.16:8080/user/changePass',{
                method: 'PUT',
                headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${accessToken}`
