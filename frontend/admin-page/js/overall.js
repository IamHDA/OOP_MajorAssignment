...
async function getThisMonthRevenue(){
    ...
    let response = await fetch('http://100.126.61.16:8080/order/admin/getThisMonthRevenue', {
        method: 'GET',
        headers: {
...
async function getThisMonthUsers() {
    ...
    await checkAccessTokenIsvalid();
    let accessToken = localStorage.getItem("accessToken");
    let response = await fetch('http://100.126.61.16:8080/user/admin/getThisMonthUsers', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`
...
async function getCountThisMonthOrder(){
    ...
    await checkAccessTokenIsvalid();
    let accessToken = localStorage.getItem("accessToken");
    let response = await fetch('http://100.126.61.16:8080/order/admin/countThisMonthOrder', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`
